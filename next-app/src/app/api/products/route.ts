...
export async function POST(req: Request) {
    try {
        ...
        const { productId, productName, price, quantityAvailable } = body;

        // Validate input
        if (!productId || !productName || price <= 0 || quantityAvailable < 0) {
        ...
        // Save the product
        const newProduct = new Product({ productId, productName, price, quantityAvailable });
